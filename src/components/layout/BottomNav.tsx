...
export function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-card via-card to-card/95 border-t border-primary/20 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] pb-safe-bottom print:!hidden" style={{ display: 'var(--nav-display, flex)' }}>
      <div className="flex items-center justify-evenly w-full py-2.5 px-2">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
